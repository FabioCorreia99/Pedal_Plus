...
import { UserMinus, ShieldCheck, ShieldOff } from 'lucide-react-native';
...
export default function GroupMembersList({ 
  ...
  const fetchMembers = async () => {
    try {
      ...
      const cleanMembers = data.map((m: any) => ({
        ...m,
        profiles: m.profiles || { full_name: null, username: 'Utilizador', avatar_url: null }
      }));
      ...
      <View style={styles.memberCard}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.name}>
            {item.profiles.full_name || item.profiles.username || 'Desconhecido'} {isMe && '(Tu)'}
          </Text>
          
          <Text style={[
            styles.role, 
            displayAdmin && { color: COLORS.primaryOrange, fontWeight: 'bold' }
          ]}>
        ...
        {iAmAdmin && !isMe && !targetIsAdminOrOwner && (
          <View style={styles.actions}>
            <TouchableOpacity onPress={() => handlePromote(item)} style={[styles.iconBtn, { backgroundColor: '#EBF5FF' }]}>
              <ShieldCheck size={18} color={COLORS.blue} />
            </TouchableOpacity>
            <TouchableOpacity onPress={() => handleKick(item)} style={[styles.iconBtn, { backgroundColor: '#FEF2F2' }]}>
        ...
        {iAmOwner && !isMe && targetIsAdminOrOwner && !isOwner && (
           <View style={styles.actions}>
             <TouchableOpacity onPress={() => handleDemote(item)} style={[styles.iconBtn, { backgroundColor: '#FFF4E5' }]}>
  ...
  return (
    <FlatList
      ...
      ListEmptyComponent={!loading ? <Text style={styles.emptyText}>Sem membros.</Text> : null}
      ListFooterComponent={loading ? <ActivityIndicator color={COLORS.primaryGreen} style={{marginTop: 20}} /> : null}
    />
  );
}
